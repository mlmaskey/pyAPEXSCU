...
class simAPEX:
    def __init__(
        ...
        self.pem = ['CODAD', 'RMSEAD', 'NRMSEAD', 'NSEAD', 'PBIASAD', 'APBIASAD', 'IOAAD', 'OF1AD', 'OF2AD',
        ...
        if (model_mode=="calibration"):
            df_PEM_runoff, df_PEM_sediment_YSD, df_PEM_sediment_USLE = pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem)
            df_PEM_sediment_MUSL, df_PEM_sediment_REMX, df_PEM_sediment_MUSS = pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem)
            df_PEM_sediment_MUST, df_PEM_sediment_RUS2, df_PEM_sediment_RUSL = pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem), pd.DataFrame(columns = self.pem)
        else: 
        ...
        for i in range (id_start, n_simul):
            try:
                ...
                if os.name == 'nt':
                    ...
                    print(p.stderr)
                    print(p.stdout)
                print(f'Completed simulation {(i+1)}/{n_simul} in {round((datetime.now() - t2).total_seconds(), 3)} seconds')
                ...
                df_PEM_runoff = do_validate_fill(model_mode, df_SAD, df_PEM_runoff, in_obj, 'WYLD', 'runoff', i, warm_years, calib_years)
                if is_pesticide:
                    df_PEM_sediment_YSD = do_validate_fill(model_mode, df_DPS, df_PEM_sediment_YSD, in_obj, 'YSD', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_USLE = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_USLE, in_obj, 'USLE', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_MUSL = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_MUSL, in_obj, 'MUSL', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_REMX = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_REMX, in_obj, 'REMX', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_MUSS = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_MUSS, in_obj, 'MUSS', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_MUST = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_MUST, in_obj, 'MUST', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_RUS2 = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_RUS2, in_obj, 'RUS2', 'sediment', i, warm_years, calib_years)
                df_PEM_sediment_RUSL = do_validate_fill(model_mode, df_SAD, df_PEM_sediment_RUSL, in_obj, 'RUSL', 'sediment', i, warm_years, calib_years)                 
                ...
                print('---------------------------------------------------------------\n')
                print(f'Completed simulation {(i+1)}/{n_simul} in {round((datetime.now() - t1).total_seconds(), 3)} seconds')
    ...
    def get_best4sa(self, scale):
        criteria = [float(self.config['COD_criteria']), float(self.config['NSE_criteria']), float(self.config['PBAIS_criteria'])]    
        ...
        f.writelines('\nIOA:       {:0.3f}'.format(best_stats[7]))
        f.writelines('\nObjValue1: {:0.3f}'.format(best_stats[8]))
        f.writelines('\nObjValue2: {:0.3f}'.format(best_stats[9]))
        f.writelines('\nModel performnce for the best set at validation\n')
        f.writelines('\nCOD:       {:0.3f}'.format(best_stats[10]))
        f.writelines('\nRMSE:      {:0.3f}'.format(best_stats[11]))
        f.writelines('\nnRMSE:     {:0.3f}'.format(best_stats[12]))
        f.writelines('\nNSE:       {:0.3f}'.format(best_stats[13]))
        f.writelines('\nPBIAS:     {:0.3f}'.format(best_stats[14]))
        f.writelines('\nIOA:       {:0.3f}'.format(best_stats[16]))
        f.writelines('\nObjValue1: {:0.3f}'.format(best_stats[17]))
        f.writelines('\nObjValue2: {:0.3f}'.format(best_stats[18]))               
        f.writelines('\n---------------------------------------------------------------')
    ...
    def get_params4ua(self, scale):
        ...
        df_stats_daily = stats_out[1]
        df_stats_daily.columns =['RunId', 'CODC', 'RMSEC', 'NRMSEC', 'NSEC', 'PBIASC', 'APBIASC',
                                  'IOAC', 'OF1C', 'OF2C', 'CODV', 'RMSEV', 'NRMSEV', 'NSEV', 
                                  'PBIASV', 'APBIASV', 'IOAV', 'OF1V', 'OF2V']
        df_stats_daily['SCALE'] = 'daily'
        df_stats_monthly = stats_out[2]
        df_stats_monthly.columns =['RunId', 'CODC', 'RMSEC', 'NRMSEC', 'NSEC', 'PBIASC', 'APBIASC',
                                  'IOAC', 'OF1C', 'OF2C', 'CODV', 'RMSEV', 'NRMSEV', 'NSEV', 
                                  'PBIASV', 'APBIASV', 'IOAV', 'OF1V', 'OF2V']
        df_stats_monthly['SCALE'] = 'monthly'
        df_stats_yearly = stats_out[3]
        df_stats_yearly.columns =['RunId', 'CODC', 'RMSEC', 'NRMSEC', 'NSEC', 'PBIASC', 'APBIASC',
                                  'IOAC', 'OF1C', 'OF2C', 'CODV', 'RMSEV', 'NRMSEV', 'NSEV', 
                                  'PBIASV', 'APBIASV', 'IOAV', 'OF1V', 'OF2V']
    ...
    def get_range (self):
        ...
        self.array_param_list = array_param_list.astype(np.float)
