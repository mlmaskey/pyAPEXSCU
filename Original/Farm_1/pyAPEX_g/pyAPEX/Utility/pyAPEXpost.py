...
class pyAPEXpost:
    ...
    def get_best_stats(df, scale, vec_criteria):
        ...
        df_pem_daily = df[(df[COD]>vec_criteria[0])&(df[NSE]>vec_criteria[1])&(df[PBAIS]<vec_criteria[2])]
        df_daily = df_pem_daily[['RunId', 'CODDC', 'RMSEDC', 'NRMSEDC', 'NSEDC', 'PBIASDC', 'APBIASDC', 'IOADC', 'OF1DC', 'OF2DC',
                             'CODDV', 'RMSEDV', 'NRMSEDV', 'NSEDV', 'PBIASDV', 'APBIASDV', 'IOADV', 'OF1DV', 'OF2DV']]
        df_monthly = df_pem_daily[['RunId', 'CODMC', 'RMSEMC', 'NRMSEMC', 'NSEMC', 'PBIASMC', 'APBIASMC', 'IOAMC', 'OF1MC', 'OF2MC', 
                             'CODMV', 'RMSEMV', 'NRMSEMV', 'NSEMV', 'PBIASMV', 'APBIASMV', 'IOAMV', 'OF1MV', 'OF2MV']]
        df_yearly = df_pem_daily[['RunId', 'CODYC', 'RMSEYC', 'NRMSEYC', 'NSEYC', 'PBIASYC', 'APBIASYC','IOAYC', 'OF1YC', 'OF2YC', 
    ...
    def get_best_stats_by_metric(df, scale, vec_criteria, citeria_metric):       
        ...
        if citeria_metric=='COD':
             ...
             df_pem = df[df[PBAIS]>vec_criteria[2]]        
        df_daily = df_pem[['RunId', 'CODDC', 'RMSEDC', 'NRMSEDC', 'NSEDC', 'PBIASDC', 'APBIASDC', 'IOADC', 'OF1DC', 'OF2DC',
                             'CODDV', 'RMSEDV', 'NRMSEDV', 'NSEDV', 'PBIASDV', 'APBIASDV', 'IOADV', 'OF1DV', 'OF2DV']]
        df_monthly = df_pem[['RunId', 'CODMC', 'RMSEMC', 'NRMSEMC', 'NSEMC', 'PBIASMC', 'APBIASMC', 'IOAMC', 'OF1MC', 'OF2MC', 
                             'CODMV', 'RMSEMV', 'NRMSEMV', 'NSEMV', 'PBIASMV', 'APBIASMV', 'IOAMV', 'OF1MV', 'OF2MV']]
        df_yearly = df_pem[['RunId', 'CODYC', 'RMSEYC', 'NRMSEYC', 'NSEYC', 'PBIASYC', 'APBIASYC','IOAYC', 'OF1YC', 'OF2YC', 
                              'CODYV', 'RMSEYV', 'NRMSEYV', 'NSEYV', 'PBIASYV', 'APBIASYV', 'IOAYV', 'OF1YV', 'OF2YV']]             
    ...
    def match_data(df_obs, df_mod):
        if df_obs.shape[0] < df_mod.shape[0]:
    ...
    def find_best(df_best, metric, stats='min'):
        value_vector = df_best[metric].values
        if stats=='min':
    ...
    def summarize_stats(df, scale, metrics = ['OF2DC', 'NSEDC', 'APBIASDC', 'CODDC'], stats = ['min', 'max', 'min', 'max']):
        from Utility.pyAPEXpost import pyAPEXpost as ap
       # Objective function based
        ...
        best_stats.columns=['RunId', 'CODC', 'RMSEC', 'NRMSEC', 'NSEC', 'PBIASC', 'APBIASC', 'IOAC', 'OF1C', 'OF2C',
                                    'CODV', 'RMSEV', 'NRMSEV', 'NSEV', 'PBIASV', 'APBIASV', 'IOAV', 'OF1V', 'OF2V', 'SCALE']
    ...
    def getSOBOL_FASTindex(param_list, list_bound, Y):
        ...
        df_fast_total.columns = ['Senstivity Index']
        df_fast_total['Order'] = 'Total'
        df_fast_total['Method'] = 'FAST'
        df_fast_First = pd.DataFrame(total.S1)
        df_fast_First.columns = ['Senstivity Index']
        df_fast_First['Order'] = 'First'
        df_fast_First['Method'] = 'FAST'
        df_SI_First = pd.concat([df_Sobol_first, df_fast_First], axis=0)
        df_SI_total = pd.concat([df_Sobol_total, df_fast_total], axis=0)
        df_SI_First['PARAM'] = df_SI_First.index
        df_SI_total['PARAM'] = df_SI_total.index
        return df_SI_First, df_SI_total
