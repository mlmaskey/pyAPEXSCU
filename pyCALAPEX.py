...
class calAPEX:
    def __init__(self, src_dir, config, cluster_dir, site, scenario, obs_attribute, mod_attribute, out_dir):
        ...
        if obs_attribute == 'runoff':
            self.criteria = [float(config['COD_criteria']), float(config['NSE_criteria']),
                             float(config['PBAIS_criteria'])]
        elif obs_attribute == 'sediment':
            self.criteria = [float(config['COD_criteria_sediment']), float(config['NSE_criteria_sediment']),
                             float(config['PBAIS_criteria_sediment'])]
        ...
        self.df_param_best = pd.concat(
            [self.df_param_best_daily, self.df_param_best_monthly, self.df_param_best_yearly], axis=0)
        ...
        df_model_outlet_monthly, df_calibration_monthly = self.final_data(scale='monthly', metric='OF',
                                                                          location='outlet')
        df_model_outlet_yearly, df_calibration_yearly = self.final_data(scale='yearly', metric='OF', location='outlet')
        self.df_model_out_OF = pd.concat([df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly],
                                         axis=0)
        ...
        self.df_model_calibration = pd.concat([df_calibration_daily, df_calibration_monthly, df_calibration_yearly],
                                              axis=0)
        ...
        self.df_model_basin_OF = pd.concat([df_model_basin_daily, df_model_basin_monthly, df_model_basin_yearly],
                                           axis=0)
        ...
        self.df_model_annual_OF = pd.concat([df_model_annual_daily, df_model_annual_monthly, df_model_annual_yearly],
                                            axis=0)
        ...
        df_model_outlet_monthly, df_calibration_monthly = self.final_data(scale='monthly', metric='NSE',
                                                                          location='outlet')
        df_model_outlet_yearly, df_calibration_yearly = self.final_data(scale='yearly', metric='NSE', location='outlet')
        self.df_model_out_NSE = pd.concat([df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly],
                                          axis=0)
        del df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly
        ...
        self.df_model_basin_NSE = pd.concat([df_model_basin_daily, df_model_basin_monthly, df_model_basin_yearly],
                                            axis=0)
        ...
        self.df_model_annual_NSE = pd.concat([df_model_annual_daily, df_model_annual_monthly, df_model_annual_yearly],
                                             axis=0)
        ...
        df_model_outlet_monthly, df_calibration_monthly = self.final_data(scale='monthly', metric='COD',
                                                                          location='outlet')
        df_model_outlet_yearly, df_calibration_yearly = self.final_data(scale='yearly', metric='COD', location='outlet')
        self.df_model_out_COD = pd.concat([df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly],
                                          axis=0)
        del df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly
        ...
        self.df_model_basin_COD = pd.concat([df_model_basin_daily, df_model_basin_monthly, df_model_basin_yearly],
                                            axis=0)
        ...
        self.df_model_annual_COD = pd.concat([df_model_annual_daily, df_model_annual_monthly, df_model_annual_yearly],
                                             axis=0)
        ...
        df_model_outlet_monthly, df_calibration_monthly = self.final_data(scale='monthly', metric='PBIAS',
                                                                          location='outlet')
        df_model_outlet_yearly, df_calibration_yearly = self.final_data(scale='yearly', metric='PBIAS',
                                                                        location='outlet')
        self.df_model_out_PBIAS = pd.concat([df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly],
                                            axis=0)
        del df_model_outlet_daily, df_model_outlet_monthly, df_model_outlet_yearly
        ...
        self.df_model_basin_PBIAS = pd.concat([df_model_basin_daily, df_model_basin_monthly, df_model_basin_yearly],
                                              axis=0)
        ...
        self.df_model_annual_PBIAS = pd.concat([df_model_annual_daily, df_model_annual_monthly, df_model_annual_yearly],
                                               axis=0)
    ...
    def extract_objective_function(self, scale):
        if scale == 'daily':
            df = self.daily2daily
            metrics = ['OF2DC', 'NSEDC', 'PBIASDC', 'CODDC']
        elif scale == 'monthly':
            df = self.monthly2monthly
            metrics = ['OF2MC', 'NSEMC', 'PBIASMC', 'CODMC']
        else:
            df = self.yearly2yearly
            metrics = ['OF2YC', 'NSEYC', 'PBIASYC', 'CODYC']
