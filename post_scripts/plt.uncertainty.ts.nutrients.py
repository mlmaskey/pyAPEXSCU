...
def plot_uncertainty_ts(site, scenario, attribute, ax, title, x_lab, y_lab):
    ...
    data_best = data_best.resample('Y').sum()
    data_best.index = data_best.index.year
...
_, lim1Nn, _ = plot_uncertainty_ts(site='Farm_1', scenario='non_grazing', attribute='TN', x_lab='',
                                   y_lab='Total nitrogen, kg/ha', ax=axes[0, 0], title='Without grazing')
axes[0, 0].legend_.remove()
_, lim1Ng, _ = plot_uncertainty_ts(site='Farm_1', scenario='grazing', attribute='TN', x_lab='',
                                   y_lab='',  ax=axes[0, 1], title='With grazing')
axes[0, 1].legend_.remove()
min_pk1 = np.max([lim1Nn[0], lim1Ng[0]])
max_pk1 = np.max([lim1Nn[1], lim1Ng[1]])
axes[0, 0].set_ylim(min_pk1, max_pk1)
axes[0, 1].set_ylim(min_pk1, max_pk1)
del min_pk1, max_pk1
del lim1Nn, lim1Ng
_, lim1Pn, _ = plot_uncertainty_ts(site='Farm_1', scenario='non_grazing', attribute='TP',  x_lab='Year',
                                   y_lab='Total phosphorus, kg/ha',   ax=axes[1, 0], title='')
axes[1, 0].legend_.remove()
_, lim1Pg, _ = plot_uncertainty_ts(site='Farm_1', scenario='grazing', attribute='TP',  x_lab='Year',
                                   y_lab='', ax=axes[1, 1], title='')
axes[1, 1].legend_.remove()
min_pk1 = np.max([lim1Pn[0], lim1Pg[0]])
max_pk1 = np.max([lim1Pn[1], lim1Pg[1]])
axes[1, 0].set_ylim(min_pk1, max_pk1)
axes[1, 1].set_ylim(min_pk1, max_pk1)
del min_pk1, max_pk1
del lim1Pn, lim1Pg
axes[1, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.20), ncol=6,  prop={'size': 12})
plt.tight_layout()
plt.savefig(os.path.join(graph_dir, 'Farm_1_Uncertainty_TN_ts.png'), dpi=600, bbox_inches="tight")
plt.show()

fig, axes = plt.subplots(2, 2, figsize=(20, 10), sharex=True, sharey=False)
_, lim1Nn, _ = plot_uncertainty_ts(site='Farm_8', scenario='non_grazing', attribute='TN', x_lab='',
                                   y_lab='Total nitrogen, kg/ha', ax=axes[0, 0], title='Without grazing')
axes[0, 0].legend_.remove()
_, lim1Ng, _ = plot_uncertainty_ts(site='Farm_8', scenario='grazing', attribute='TN', x_lab='',
                                   y_lab='',  ax=axes[0, 1], title='With grazing')
axes[0, 1].legend_.remove()
min_pk1 = np.max([lim1Nn[0], lim1Ng[0]])
max_pk1 = np.max([lim1Nn[1], lim1Ng[1]])
...
del min_pk1, max_pk1
del lim1Nn, lim1Ng
_, lim1Pn, _ = plot_uncertainty_ts(site='Farm_8', scenario='non_grazing', attribute='TP',  x_lab='Year',
                                   y_lab='Total phosphorus, kg/ha',   ax=axes[1, 0], title='')
axes[1, 0].legend_.remove()
_, lim1Pg, _ = plot_uncertainty_ts(site='Farm_8', scenario='grazing', attribute='TP',  x_lab='Year',
                                   y_lab='', ax=axes[1, 1], title='')
axes[1, 1].legend_.remove()
min_pk1 = np.max([lim1Pn[0], lim1Pg[0]])
max_pk1 = np.max([lim1Pn[1], lim1Pg[1]])
...
plt.tight_layout()
plt.savefig(os.path.join(graph_dir, 'Farm_8_Uncertainty_TN_ts.png'), dpi=600, bbox_inches="tight")
plt.show()